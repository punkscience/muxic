...
var copyCmd = &cobra.Command{
	Use:   "copy",
	Short: "Copies music files to a specified destination, organizing them by metadata.",
	Long: `Copies music files from a source folder to a destination folder. It uses
metadata (tags) to create an organized folder layout (Artist/Album/Track).
File names are cleaned and standardized.
The --dry-run flag simulates operations without making changes.`,
	Run: func(cmd *cobra.Command, args []string) {
		runMusicOperation(cmd, false)
	},
}

func runMusicOperation(cmd *cobra.Command, destructive bool) {
			...
			for file := range filesChan {
				...
				if err != nil {
					if errors.Is(err, movemusic.ErrFileAlreadyExists) {
						// not a critical error, just a skip
					} else {
